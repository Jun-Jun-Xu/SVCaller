...
//            BufferedWriter bw = IOUtils.getTextGzipWriter(new File("/Users/xujun/Desktop/Tempory/lib.txt.gz").getAbsolutePath());
            while(r.hasNext()){
                ...
                if (tem.getContig().replaceAll("[a-zA-Z]","").equals("0"))continue;
                ...
                if (blSA){
                    ...
                    for (int i = 0; i < SAArray.length; i++){
                        ...
                        if(q<quality)continue;
                        chrSA = temp.split(",")[0].replaceAll("[a-zA-Z]","");
                        if (Integer.parseInt(chrSA)==0)continue;
                        num1 = Integer.parseInt(chrSA)-1;
...
//                }
                ...
                if (posLibI[num][index] - breakPointH < -range) index += 1;
